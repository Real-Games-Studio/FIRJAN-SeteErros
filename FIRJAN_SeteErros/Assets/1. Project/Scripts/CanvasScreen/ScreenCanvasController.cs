...
public class ScreenCanvasController : MonoBehaviour
{
    ...
    [SerializeField] private string[] screensExcludedFromTimeout = { "GameplayScreen" };
    ...
    void Update()
    {
        // Verifica se a tela atual deve ter timeout
        bool shouldHaveTimeout = currentScreen != inicialScreen && !IsScreenExcludedFromTimeout(currentScreen);
    ...
    /// <summary>
    /// Verifica se a tela está excluída do sistema de timeout
    /// </summary>
    /// <param name="screenName">Nome da tela</param>
    /// <returns>True se a tela deve ser excluída do timeout</returns>
    private bool IsScreenExcludedFromTimeout(string screenName)
    {
        if (screensExcludedFromTimeout == null) return false;

        foreach (string excludedScreen in screensExcludedFromTimeout)
        {
            if (screenName == excludedScreen)
            {
                return true;
            }
        }
